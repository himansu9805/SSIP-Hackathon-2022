    ...
    for (let i = 0; i < slotsList.length; i++) {
      async function getAvail(){
        const queryQ = query(
          ...
          where("date", "==", "" + dateObj.date() + "/" + (dateObj.month() + 1) + "/" + dateObj.year())
        );
  ...
  const bookAppointment = async() => {
    ...
    try {
      await addDoc(collection(db, "Appointments"), {
        booking_id: dateObj.format('MMMM').substring(0, 3).toUpperCase() + "_" + dateObj.date() + "_" + (slotsAvailability[slot]+1) ,
      ...
      <Box p={2}>
        <Grid container columnSpacing={ {md : 4}}>
          <Grid item xs={12} md={3} sx={{ display: "flex",flexDirection: "column", alignItems:"center", justifyContent:"center", padding: 0 }}>
            <div>
              ...
              <div className='datePickContainer'>
                ...
                <Button ref={btnRef} style={{ display: "none" }} onClick={nakliClick}>effefef</Button>
              </div>
              <div >
                &nbsp;
              </div>
          ...
          <Grid item xs={12} md={6}>
            ...
            : (slotsLoading ? <div className="slotsContainer">
                <div style={{ display: "flex", width: "100%", flexDirection: "column" }}>
              ...
              :
              <Slots validSlots={slotsList} loading={slotDataLoad} availabilityArray={slotsAvailability} announceSlot={(i)=>{
                setSlot(i)
              }} />)
            }            
                ...
                <div className='confirmBody'>
                  ...
                  <div className='confirmDateDetail'>
                    ...
                    <span style={{ marginLeft: "05px" }}>
                       { slotsList[slot].time } - {slot===slotsList.length-1 ? "06 : 00 PM" : slotsList[slot+1]?.time } 
                    </span>
                  </div>
                  <div className='confirmDateDetail'>
                    <span>
